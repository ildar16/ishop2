    ...
    public function editAction()
    {
        if (!empty($_POST)){
            ...
            if ($product->update('product', $id)){
                ...
                $product->alias = $alias;
                \R::store($product);
                $_SESSION['success'] = 'Товар изменен';
                redirect();
            }
        ...
        $relatedProduct = \R::getAll("SELECT related_product.related_id, product.title FROM related_product JOIN product ON product.id = related_product.related_id WHERE related_product.product_id = ?", [$id]);
        $gallery = \R::getCol('SELECT img FROM gallery WHERE product_id = ?', [$id]);
        $this->setMeta("Редактирование товара {$product->title}");
        $this->set(compact('product', 'filter', 'gallery', 'relatedProduct'));
    }
...
//            debug($data['mod'], 1);
            $product->load($data);
            $product->attributes['status'] = $product->attributes['status'] ? '1' : '1';
            $product->attributes['hit'] = $product->attributes['hit'] ? '1' : '1';
            $product->getImg();
