...
class UserController extends AppController
{
    ...
    public function editAction()
    {
        if (!empty($_POST)){
            ...
            if ($user->update('user', $id)){
                $_SESSION['success'] = 'Изминения сохранены';
            }
            redirect();
        }
    ...
    public function deleteAction()
    {
        $user_id = $this->getRequestID();
        $user = \R::load('user', $user_id);
        $orders_id = \R::getAssoc("SELECT id FROM `order` WHERE user_id = {$user_id}");
        $order_products = \R::batch('order', $orders_id);
        ...
        $_SESSION['success'] = 'Пользователь и его заказы удалены';
        redirect(ADMIN . '/user');
    }
